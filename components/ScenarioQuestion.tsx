...
const ScenarioQuestion: React.FC<ScenarioQuestionProps> = ({ question, index, values, onChange }) => {
  ...
  return (
    <div className="p-4 rounded-xl bg-slate-50 shadow-sm">
      <h3 className="font-bold text-slate-700 mb-3">{question.q}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {options.map((letter) => {
          ...
          return (
            <label key={id} htmlFor={id} className="flex gap-3 items-center p-3 rounded-lg cursor-pointer hover:bg-slate-200 transition-colors has-[:checked]:bg-pink-100">
              <input
                type="checkbox"
                name={`q${index}`}
                id={id}
